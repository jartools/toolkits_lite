...
public class InternalBuilder extends ExToolkit {
	...
	public static String build(Connection conn, ResultSet rs, String pkg, String appContext, boolean batch, boolean sorted) throws Exception {
		...
		sn(sb, "public abstract class %sInternal extends InternalSupport{", tbUEn);
		sn(sb, "    static Log log = LogFactory.getLog(%sInternal.class);", tbUEn);
		...
		sn(sb, "    public static final Map<%s, %s> vars = %s();", priKeyType, tbUEn, (sorted ? "newSortedMap" : "newMap"));
		{
			...
			while (it.hasNext()) {
				...
				if (idx_size == 1) { // 单索引
					...
					if (NON_UNIQUE.equals("false")) { // 唯一数据
						sn(sb, "    public static final Map<%s, %s> varsBy%s = %s();", COLUMN_NAME_TYPE, priKeyType, COLUMN_NAME_UEN, (sorted ? "newSortedMap" : "newMap"));
					} else {
						if (COLUMN_NAME_TYPE.equals("java.util.Date"))
							continue;
						sn(sb, "    public static final Map<%s, Set<%s>> varsBy%s = %s();", COLUMN_NAME_TYPE, priKeyType, COLUMN_NAME_UEN, (sorted ? "newSortedMap" : "newMap"));
					}
					...
					String NON_UNIQUE = String.valueOf(index.get("NON_UNIQUE"));
					if (NON_UNIQUE.equals("false")) { // 唯一数据
						sn(sb, "    public static final Map<String, %s> varsBy%s = %s();", priKeyType, index1, (sorted ? "newSortedMap" : "newMap"));
					} else {
						sn(sb, "    public static final Map<String, Set<%s>> varsBy%s = %s();", priKeyType, index1, (sorted ? "newSortedMap" : "newMap"));
					}
				}
			}
		}
		sn(sb, "");
		sn(sb, "    private static final List<%s> fixedCache = newList();", tbUEn);
		sn(sb, "");

		sn(sb, "    public static void put(%s %s, boolean force){", tbUEn, tbEn);
		sn(sb, "        if(%s == null || %s.%s <= 0) return ;", tbEn, tbEn, priKey);
		sn(sb, "");
		sn(sb, "        %s %s = %s.%s;", pkBType, priKey, tbEn, priKey);
		sn(sb, "        if (cacheModel == STATIC_CACHE) {");

		if (isInt4Pk) {
			sn(sb, "            if (%s > 0 && %s <= FIXED.length) {", priKey, priKey);
			sn(sb, "                if (FIXED[%s - 1] == null || !FIXED[%s - 1].equals(%s))", priKey, priKey, tbEn);
			...
			while (it.hasNext()) {
				...
				if (idx_size == 1) { // 单索引
					...
					if (INDEX_NAME.equals("PRIMARY"))
						continue;
					if (NON_UNIQUE.equals("false")) { // 唯一数据
						sn(sb, "        { // 单-唯一索引 remove old index %s", COL_NAME);
						sn(sb, "          Object ov = %s.oldVal(Col.%s);", tbEn, COL_NAME);
						sn(sb, "          if(ov != null)");
						sn(sb, "              varsBy%s.remove(ov);", COL_NAME_UEN);
						sn(sb, "          if(ov != null || force){ // put new index");
						sn(sb, "            %s %s = %s.get%s();", bType, COL_NAME_EN, tbEn, COL_NAME_UEN);
						sn(sb, "            varsBy%s.put(%s, %s);", COL_NAME_UEN, COL_NAME_EN, priKey);
						sn(sb, "          }");
						sn(sb, "        }");
					} else {
						if (bType.equals("java.util.Date"))
							continue;
						p = p + 1;
						sn(sb, "        { // 单-非唯一索引 remove old index %s", COL_NAME);
						sn(sb, "          Object ov = %s.oldVal(Col.%s);", tbEn, COL_NAME);
						sn(sb, "          if(ov != null) {");
						sn(sb, "              %s _val = (%s) ov;", COL_NAME_TYPE, COL_NAME_TYPE);
						sn(sb, "              Set m%d = varsBy%s.get(_val);", p, COL_NAME_UEN);
						sn(sb, "              if(m%d != null) {", p);
						sn(sb, "                  m%d.remove(%s);", p, priKey);
						sn(sb, "                  if(m%d.isEmpty())", p);
						sn(sb, "                      varsBy%s.remove(_val);", COL_NAME_UEN, p);
						sn(sb, "              }");
						sn(sb, "          }");
						sn(sb, "          if(ov != null || force){ // put new index");
						sn(sb, "            %s %s = %s.get%s();", bType, COL_NAME_EN, tbEn, COL_NAME_UEN);
						sn(sb, "            Set m%d = varsBy%s.get(%s);", p, COL_NAME_UEN, COL_NAME_EN);
						sn(sb, "            if(m%d == null){", p);
						sn(sb, "                m%d = %s();", p, (sorted ? "newSortedSet" : "newSet"));
						sn(sb, "                varsBy%s.put(%s, m%d);", COL_NAME_UEN, COL_NAME_EN, p);
						sn(sb, "            }", p);
					...
					if (NON_UNIQUE.equals("false")) { // 唯一数据
						...
						if (tmp.len() > 2)
							tmp.removeRight(2);
						sn(sb, "          boolean bChanged = %s.inChanged(%s);", tbEn, tmp.str());
						sn(sb, "          if(bChanged) { // 多-唯一索引 remove old index");
						tmp.close();
						for (Map<String, Object> map : idx) {
							String COL_NAME = MapEx.get(map, "COLUMN_NAME");
							String COL_NAME_EN = PinYin.getShortPinYin(COL_NAME);
							sn(sb, "            Object v%s = %s.oldOrNew(Col.%s);", COL_NAME_EN, tbEn, COL_NAME);
						...
						sn(sb, "          }");
						sn(sb, "          if(bChanged || force) { // put new index");
						for (Map<String, Object> map : idx) {
							String COL_NAME = MapEx.get(map, "COLUMN_NAME");
							String COL_NAME_EN = PinYin.getShortPinYin(COL_NAME);
							String COL_NAME_UEN = StrEx.upperN1(COL_NAME_EN);
							String COL_NAME_TYPE = JTypeMysql.getType(rsmd, COL_NAME);
							String bType = JTypeMysql.getBasicType(COL_NAME_TYPE);
							sn(sb, "              %s v%s = %s.get%s();", bType, COL_NAME_EN, tbEn, COL_NAME_UEN);
						...
						sn(sb, "              varsBy%s.put(vkey, %s);", index1, priKey);
						sn(sb, "          }");
						sn(sb, "        }");
					} else {
						String skey = "com.bowlong.lang.PStr.b().a(";
						...
						if (tmp.len() > 2)
							tmp.removeRight(2);
						sn(sb, "          boolean bChanged = %s.inChanged(%s);", tbEn, tmp.str());
						sn(sb, "          if(bChanged){ // 多-非唯一索引 remove old index");
						tmp.close();
						for (Map<String, Object> map : idx) {
							String COL_NAME = MapEx.get(map, "COLUMN_NAME");
							String COL_NAME_EN = PinYin.getShortPinYin(COL_NAME);
						...
						skey1 = skey1 + ").e()";
						sn(sb, "              String okey = %s;", skey1);
						sn(sb, "              Set m%d = varsBy%s.get(okey);", p, index1);
						sn(sb, "              if(m%d != null) {", p);
						sn(sb, "                  m%d.remove(%s);", p, priKey);
						sn(sb, "                  if(m%d.isEmpty())", p);
						sn(sb, "                       varsBy%s.remove(okey);", index1);
						sn(sb, "              }", p);
						sn(sb, "          }");
						sn(sb, "          if(bChanged || force) { // put new index", INDEX_NAME);
						for (Map<String, Object> map : idx) {
							String COL_NAME = MapEx.get(map, "COLUMN_NAME");
							String COL_NAME_EN = PinYin.getShortPinYin(COL_NAME);
							String COL_NAME_UEN = StrEx.upperN1(COL_NAME_EN);
		...
		sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
		...
		sn(sb, "    public static int count(%sDAO DAO, String TABLENAME2){", tbUEn);
		...
		sn(sb, "    public static void relocate(%sDAO DAO, String TABLENAME2) {", tbUEn);
		sn(sb, "        DAO().TABLENAME = TABLENAME2;");
		sn(sb, "    }");
		sn(sb, "");
		...
		sn(sb, "    public static void createTable(%sDAO DAO, String TABLENAME2) {", tbUEn);
		sn(sb, "        DAO.createTable(TABLENAME2);");
		sn(sb, "    }");
		sn(sb, "");
		...
		sn(sb, "    public static void createNoUniqueTable(%sDAO DAO, String TABLENAME2) {", tbUEn);
		sn(sb, "        DAO.createNoUniqueTable(TABLENAME2);");
		sn(sb, "    }");
		sn(sb, "");
		...
		// sortBy
		sn(sb, "    public static List<%s> sortZh(List<%s> %ss, final String key) {", tbUEn, tbUEn, tbEn);
		sn(sb, "        Collections.sort(%ss, new Comparator<%s>() {", tbEn, tbUEn, tbUEn);
		...
		sn(sb, "    public static List<%s> sort(List<%s> %ss, final String key) {", tbUEn, tbUEn, tbEn);
		sn(sb, "        Collections.sort(%ss, new Comparator<%s>() {", tbEn, tbUEn, tbUEn);
		sn(sb, "            public int compare(%s o1, %s o2) {", tbUEn, tbUEn);
		...
		sn(sb, "    public static List<%s> sort(List<%s> %ss){", tbUEn, tbUEn, tbEn);
		sn(sb, "        Collections.sort(%ss, new Comparator<%s>(){", tbEn, tbUEn);
		sn(sb, "            public int compare(%s o1, %s o2) {", tbUEn, tbUEn);
		...
		sn(sb, "    public static List<%s> sortReverse(List<%s> %ss){", tbUEn, tbUEn, tbEn);
		sn(sb, "        Collections.sort(%ss, new Comparator<%s>(){", tbEn, tbUEn);
		sn(sb, "            public int compare(%s o1, %s o2) {", tbUEn, tbUEn);
		sn(sb, "                Object v1 = o1.%s;", priKey);
		sn(sb, "                Object v2 = o2.%s;", priKey);
...

		{
			...
			while (it.hasNext()) {
				...
				if (idx_size == 1) { // 单索引
					...
					if (INDEX_NAME.equals("PRIMARY"))
						continue;
					sn(sb, "    public static List<%s> sort%s(List<%s> %ss){", tbUEn, COLUMN_NAME_UEN, tbUEn, tbEn);
					sn(sb, "        Collections.sort(%ss, new Comparator<%s>(){", tbEn, tbUEn);
					sn(sb, "            public int compare(%s o1, %s o2) {", tbUEn, tbUEn);
					...
					sn(sb, "    }");
					sn(sb, "");
					sn(sb, "    public static List<%s> sort%sRo(List<%s> %ss){", tbUEn, COLUMN_NAME_UEN, tbUEn, tbEn);
					sn(sb, "        Collections.sort(%ss, new Comparator<%s>(){", tbEn, tbUEn);
					sn(sb, "            public int compare(%s o1, %s o2) {", tbUEn, tbUEn);
					sn(sb, "                Object v1 = o1.get%s();", COLUMN_NAME_UEN);
					sn(sb, "                Object v2 = o2.get%s();", COLUMN_NAME_UEN);
		...
		sn(sb, "    public static %s insert(%sDAO DAO, %s %s) {", tbUEn, tbUEn, tbUEn, tbEn);
		sn(sb, "        return insert(DAO, %s, DAO.TABLENAME);", tbEn);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s insert(%s %s, String TABLENAME2) {", tbUEn, tbUEn, tbEn);
		sn(sb, "        %sDAO DAO = DAO();", tbUEn);
		sn(sb, "        return insert(DAO, %s, TABLENAME2);", tbEn);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s insert(%sDAO DAO, %s %s, String TABLENAME2) {", tbUEn, tbUEn, tbUEn, tbEn);
		if (isInt4Pk) {
		...
		sn(sb, "    public static %s asyncInsert(%sDAO DAO, %s %s) {", tbUEn, tbUEn, tbUEn, tbEn);
		sn(sb, "        return asyncInsert(DAO, %s, DAO.TABLENAME);", tbEn);
		sn(sb, "    }");

		sn(sb, "    public static %s asyncInsert(%s %s, String TABLENAME2) {", tbUEn, tbUEn, tbEn);
		sn(sb, "        %sDAO DAO = DAO();", tbUEn);
		sn(sb, "        return asyncInsert(DAO, %s, TABLENAME2);", tbEn);
		sn(sb, "    }");

		sn(sb, "    public static %s asyncInsert(%sDAO DAO, %s %s, String TABLENAME2) {", tbUEn, tbUEn, tbUEn, tbEn);
		if (isInt4Pk) {
			sn(sb, "        if (cacheModel == STATIC_CACHE && LASTID.intValue() >= MAX) {");
			sn(sb, "            log.error(\"The cacheModel = STATIC_CACHE is Full.\");");
			sn(sb, "            return null;");
		...
		sn(sb, "    public static %s insert2(%sDAO DAO, %s %s) {", tbUEn, tbUEn, tbUEn, tbEn);
		sn(sb, "        return insert2(DAO, %s, DAO.TABLENAME);", tbEn);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s insert2(%s %s, String TABLENAME2) {", tbUEn, tbUEn, tbEn);
		sn(sb, "        %sDAO DAO = DAO();", tbUEn);
		sn(sb, "        return insert2(DAO, %s, TABLENAME2);", tbEn);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s insert2(%sDAO DAO, %s %s, String TABLENAME2) {", tbUEn, tbUEn, tbUEn, tbEn);
		if (isInt4Pk) {
			sn(sb, "        if (cacheModel == STATIC_CACHE && LASTID.intValue() >= MAX) {");
			sn(sb, "            log.error(\"The cacheModel = STATIC_CACHE is Full.\");");
			sn(sb, "            return null;");
		...
		sn(sb, "    public static %s asyncInsert2(%sDAO DAO, %s %s) {", tbUEn, tbUEn, tbUEn, tbEn);
		sn(sb, "        return asyncInsert2(DAO, %s, DAO.TABLENAME);", tbEn);
		sn(sb, "    }");

		sn(sb, "    public static %s asyncInsert2(%s %s, String TABLENAME2) {", tbUEn, tbUEn, tbEn);
		sn(sb, "        %sDAO DAO = DAO();", tbUEn);
		sn(sb, "        return asyncInsert2(DAO, %s, TABLENAME2);", tbEn);
		sn(sb, "    }");

		sn(sb, "    public static %s asyncInsert2(%sDAO DAO, %s %s, String TABLENAME2) {", tbUEn, tbUEn, tbUEn, tbEn);
		if (isInt4Pk) {
			sn(sb, "        if (cacheModel == STATIC_CACHE && LASTID.intValue() >= MAX) {");
			sn(sb, "            log.error(\"The cacheModel = STATIC_CACHE is Full.\");");
			sn(sb, "            return null;");
		...
		if (batch) { // 批处理
			sn(sb, "    public static int[] insert(List<%s> %ss) {", tbUEn, tbEn);
			sn(sb, "        %sDAO DAO = DAO();", tbUEn);
			sn(sb, "        return insert(DAO, %ss, DAO.TABLENAME);", tbEn);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int[] insert(%sDAO DAO, List<%s> %ss) {", tbUEn, tbUEn, tbEn);
			sn(sb, "        return insert(DAO, %ss, DAO.TABLENAME);", tbEn);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int[] insert(List<%s> %ss, String TABLENAME2) {", tbUEn, tbEn, tbUEn, tbEn);
			sn(sb, "        %sDAO DAO = DAO();", tbUEn);
			sn(sb, "        return insert(DAO, %ss, TABLENAME2);", tbEn);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int[] insert(%sDAO DAO, List<%s> %ss, String TABLENAME2) {", tbUEn, tbUEn, tbEn);
			if (isInt4Pk) {
				sn(sb, "        if (cacheModel == STATIC_CACHE && LASTID.intValue() >= MAX) {");
				sn(sb, "            log.error(\"The cacheModel = STATIC_CACHE is Full.\");");
				sn(sb, "            return null;");
			...
			sn(sb, "            ret[n++] = (%s == null) ? 0 : (int)%s.%s;", tbEn, tbEn, priKey);
			sn(sb, "        }");
			sn(sb, "        return ret;");
			sn(sb, "    }");
			sn(sb, "");
		...
		sn(sb, "    public static int delete(%sDAO DAO, %s %s) {", tbUEn, pkBType, priKey);
		sn(sb, "        return delete(DAO, %s, DAO.TABLENAME);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static int delete(%s %s, String TABLENAME2) {", pkBType, priKey);
		sn(sb, "        %sDAO DAO = DAO();", tbUEn);
		sn(sb, "        return delete(DAO, %s, TABLENAME2);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static int delete(%sDAO DAO, %s %s, String TABLENAME2) {", tbUEn, pkBType, priKey);
		sn(sb, "        int n = 1;");
		sn(sb, "        if(cacheModel != FULL_MEMORY){");
		...
		sn(sb, "    public static void asyncDelete(%sDAO DAO, %s %s) {", tbUEn, pkBType, priKey);
		sn(sb, "        asyncDelete(DAO, %s, DAO.TABLENAME);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static void asyncDelete(%s %s, String TABLENAME2) {", pkBType, priKey);
		sn(sb, "        %sDAO DAO = DAO();", tbUEn);
		sn(sb, "        asyncDelete(DAO, %s, TABLENAME2);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static void asyncDelete(%sDAO DAO, %s %s, String TABLENAME2) {", tbUEn, pkBType, priKey);
		sn(sb, "        if(cacheModel != FULL_MEMORY){");
		...
		if (batch) { // 批处理
			sn(sb, "    public static int[] delete(%s[] %ss) {", pkBType, priKey);
			sn(sb, "        %sDAO DAO = DAO();", tbUEn);
			sn(sb, "        return delete(DAO, %ss, DAO.TABLENAME);", priKey);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int[] delete(%sDAO DAO, %s[] %ss) {", tbUEn, pkBType, priKey);
			sn(sb, "        return delete(DAO, %ss, DAO.TABLENAME);", priKey);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int[] delete(%s[] %ss,String TABLENAME2) {", pkBType, priKey);
			sn(sb, "        %sDAO DAO = DAO();", tbUEn);
			sn(sb, "        return delete(DAO, %ss, TABLENAME2);", priKey);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int[] delete(%sDAO DAO, %s[] %ss,String TABLENAME2) {", tbUEn, pkBType, priKey);
			sn(sb, "        if(cacheModel == NO_CACHE){");
			sn(sb, "            return DAO.deleteByKey(%ss, TABLENAME2);", priKey);
			sn(sb, "        }");
			sn(sb, "        int[] ret = new int[%ss.length];", priKey);
			sn(sb, "        int n = 0;");
			sn(sb, "        for(%s %s : %ss){", pkBType, priKey, priKey);
			sn(sb, "            ret[n++] = delete(DAO, %s, TABLENAME2);", priKey);
			sn(sb, "        }");
			sn(sb, "        return ret;");
			sn(sb, "    }");
			sn(sb, "");
		...
		if (batch) {
			sn(sb, "    public static int deleteIn(List<%s> keys) {", priKeyType);
			sn(sb, "        %sDAO DAO = DAO();", tbUEn);
			sn(sb, "        return deleteIn(keys, DAO, DAO.TABLENAME);");
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int deleteIn(List<%s> keys, %sDAO DAO) {", priKeyType, tbUEn);
			sn(sb, "        return deleteIn(keys, DAO, DAO.TABLENAME);");
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int deleteIn(List<%s> keys, String TABLENAME2) {", priKeyType);
			sn(sb, "        %sDAO DAO = DAO();", tbUEn);
			sn(sb, "        return deleteIn(keys, DAO, TABLENAME2);");
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int deleteIn(final List<%s> keys, final %sDAO DAO, final String TABLENAME2) {", priKeyType, tbUEn);
			...
			sn(sb, "    public static int deleteWith(List<%s> beans, %sDAO DAO) {", tbUEn, tbUEn);
			sn(sb, "        return deleteWith(beans, DAO, DAO.TABLENAME);");
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int deleteWith(List<%s> beans, String TABLENAME2) {", tbUEn);
			sn(sb, "        %sDAO DAO = DAO();", tbUEn);
			sn(sb, "        return deleteWith(beans, DAO, TABLENAME2);");
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static int deleteWith(final List<%s> beans, final %sDAO DAO, final String TABLENAME2) {", tbUEn, tbUEn);
		...
		sn(sb, "    public static List<%s> getAll(final %sDAO DAO, final String TABLENAME2) {", tbUEn, tbUEn);
		sn(sb, "        if(cacheModel == NO_CACHE){");
		sn(sb, "            return DAO.selectAll(TABLENAME2);");
		sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY ");
		sn(sb, "            List<%s> result = getNoSortAll(DAO, TABLENAME2);", tbUEn);
		...
		sn(sb, "    public static List<%s> getNoSortAll(%sDAO DAO) {", tbUEn, tbUEn);
		sn(sb, "        return getNoSortAll(DAO, DAO.TABLENAME);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getNoSortAll(String TABLENAME2) {", tbUEn);
		sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
		sn(sb, "        return getNoSortAll(DAO, TABLENAME2);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getNoSortAll(final %sDAO DAO, final String TABLENAME2) {", tbUEn, tbUEn);
		sn(sb, "        if(cacheModel == NO_CACHE){");
		sn(sb, "            return DAO.selectAll(TABLENAME2);");
		sn(sb, "        } else if (cacheModel == STATIC_CACHE) {");
		...
		if (isInt4Pk) {
			...
			sn(sb, "                if (%s != null) result.add((%s)(i + 1));", tbEn, pkBType);
			sn(sb, "            }");
			sn(sb, "            return result;");
			sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY ");
		...
		sn(sb, "    public static List<%s> getPks(%sDAO DAO) {", priKeyType, tbUEn);
		sn(sb, "        return getPks(DAO, DAO.TABLENAME);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getPks(String TABLENAME2) {", priKeyType);
		sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
		sn(sb, "        return getPks(DAO, TABLENAME2);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getPks(final %sDAO DAO, final String TABLENAME2) {", priKeyType, tbUEn);
		sn(sb, "        if ( cacheModel == NO_CACHE) { ");
		...
		sn(sb, "    public static List<Map> getInIndex(final %sDAO DAO, final String TABLENAME2) {", tbUEn);
		sn(sb, "        return DAO.selectInIndex(TABLENAME2);");
		sn(sb, "    }");
		sn(sb, "");

		if (batch) {
			sn(sb, "    public static List<%s> getIn(List<%s> keys) {", tbUEn, priKeyType);
			sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
			sn(sb, "        return getIn(keys, DAO, DAO.TABLENAME);", priKey);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static List<%s> getIn(List<%s> keys, %sDAO DAO) {", tbUEn, priKeyType, tbUEn);
			sn(sb, "        return getIn(keys, DAO, DAO.TABLENAME);", priKey);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static List<%s> getIn(List<%s> keys, String TABLENAME2) {", tbUEn, priKeyType);
			sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
			sn(sb, "        return getIn(keys, DAO, TABLENAME2);", priKey);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static List<%s> getIn(final List<%s> keys, final %sDAO DAO, final String TABLENAME2) {", tbUEn, priKeyType, tbUEn);
			sn(sb, "        if(keys == null || keys.isEmpty()) return newList();");
			sn(sb, "        if( cacheModel == NO_CACHE ){");
			sn(sb, "            return DAO.selectIn(keys, TABLENAME2);");
			sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY ");
			sn(sb, "            List<%s> result = getNoSortIn(keys, DAO, TABLENAME2);", tbUEn);
			// sn(sb, " return sort(result);");
			sn(sb, "            return result;");
			sn(sb, "        }");
			sn(sb, "    }");
			...
			sn(sb, "    public static List<%s> getNoSortIn(List<%s> keys) {", tbUEn, priKeyType);
			sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
			sn(sb, "        return getNoSortIn(keys, DAO, DAO.TABLENAME);", priKey);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static List<%s> getNoSortIn(List<%s> keys, %sDAO DAO) {", tbUEn, priKeyType, tbUEn);
			sn(sb, "        return getNoSortIn(keys, DAO, DAO.TABLENAME);", priKey);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static List<%s> getNoSortIn(List<%s> keys, String TABLENAME2) {", tbUEn, priKeyType);
			sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
			sn(sb, "        return getNoSortIn(keys, DAO, TABLENAME2);", priKey);
			sn(sb, "    }");
			sn(sb, "");

			sn(sb, "    public static List<%s> getNoSortIn(final List<%s> keys, final %sDAO DAO, final String TABLENAME2) {", tbUEn, priKeyType, tbUEn);
			sn(sb, "        if(keys == null || keys.isEmpty()) return newList();");
			sn(sb, "        if( cacheModel == NO_CACHE ){");
			sn(sb, "            return DAO.selectIn(keys, TABLENAME2);");
			...
			sn(sb, "                %s %s = getByKeyFromMemory(key);", tbUEn, tbEn);
		...
		sn(sb, "    public static List<%s> getLast(%sDAO DAO, int num) {", tbUEn, tbUEn);
		sn(sb, "        return getLast(DAO, num, DAO.TABLENAME);");
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getLast(int num, String TABLENAME2) {", tbUEn);
		sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
		sn(sb, "        return getLast(DAO, num, TABLENAME2);");
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getLast(final %sDAO DAO, final int num, final String TABLENAME2) {", tbUEn, tbUEn);
		sn(sb, "        if( cacheModel == NO_CACHE ){");
		...
		sn(sb, "    public static %s last(final %sDAO DAO, final String TABLENAME2) {", tbUEn, tbUEn);
		...
		sn(sb, "    public static %s maxId(final %sDAO DAO, final String TABLENAME2) {", pkBType, tbUEn);
		sn(sb, "        if( cacheModel == NO_CACHE ){");
		...
		sn(sb, "    public static List<%s> getGtKey(%s %s) {", tbUEn, pkBType, priKey);
		sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
		sn(sb, "        return getGtKey(DAO, %s, DAO.TABLENAME);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getGtKey(%sDAO DAO, %s %s) {", tbUEn, tbUEn, pkBType, priKey);
		sn(sb, "        return getGtKey(DAO, %s, DAO.TABLENAME);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getGtKey(%s %s, String TABLENAME2) {", tbUEn, pkBType, priKey);
		sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
		sn(sb, "        return getGtKey(DAO, %s, TABLENAME2);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getGtKey(final %sDAO DAO, final %s %s,final String TABLENAME2) {", tbUEn, tbUEn, pkBType, priKey);
		sn(sb, "        if( cacheModel == NO_CACHE ){");
		...
		sn(sb, "            for (%s %s : %ss) {", tbUEn, tbEn, tbEn);
		sn(sb, "                if(%s.%s <= %s) continue;", tbEn, priKey, priKey);
		sn(sb, "                result.add(%s);", tbEn);
		sn(sb, "            }");
		sn(sb, "            return result;");
		sn(sb, "        }");
		...
		sn(sb, "    public static Future<%s> asyncGetByKey(final %s %s) {", tbUEn, pkBType, priKey);
		sn(sb, "        Future<%s> f = Async.exec(new Callable<%s>() {", tbUEn, tbUEn);
		...
		sn(sb, "    public static %s getByKey(%sDAO DAO, %s %s) {", tbUEn, tbUEn, pkBType, priKey);
		sn(sb, "        return getByKey(DAO, %s, DAO.TABLENAME);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s getByKey(%s %s, String TABLENAME2) {", tbUEn, pkBType, priKey);
		sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
		sn(sb, "        return getByKey(DAO, %s, TABLENAME2);", priKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s getByKey(final %sDAO DAO, final %s %s,final String TABLENAME2) {", tbUEn, tbUEn, pkBType, priKey);
		sn(sb, "        if(cacheModel == NO_CACHE){");
		...
		sn(sb, "    public static %s getByKeyFromMemory(final %s %s) {", tbUEn, pkBType, priKey);
		if (isInt4Pk) {
			sn(sb, "        if (cacheModel == STATIC_CACHE) {");
			sn(sb, "            if (%s < 1 || FIXED == null || FIXED.length < %s) return null;", priKey, priKey);
			sn(sb, "            return FIXED[%s - 1];", priKey);
			sn(sb, "        } else if (cacheModel == FULL_CACHE || cacheModel == FULL_MEMORY) {");
		} else {
			sn(sb, "        if (cacheModel == FULL_CACHE || cacheModel == FULL_MEMORY) {");
		}
		...
		sn(sb, "    public static %s deleteFromMemory(final %s %s) {", tbUEn, pkBType, priKey);
		sn(sb, "        %s %s;", tbUEn, tbEn);
		if (isInt4Pk) {
			sn(sb, "        if (cacheModel == STATIC_CACHE) {");
			sn(sb, "            if (%s < 1 || FIXED == null || FIXED.length < %s || FIXED[%s-1]==null) return null;", priKey, priKey, priKey);
		...
		sn(sb, "");
		{
			...
			while (it.hasNext()) {
				...
				if (idx_size == 1) { // 单索引
					...
					if (NON_UNIQUE.equals("false")) { // 唯一数据
						sn(sb, "        %s %s = %s.get%s();", bType, COL_NAME_EN, tbEn, COL_NAME_UEN);
						sn(sb, "        varsBy%s.remove(%s);", COL_NAME_UEN, COL_NAME_EN);
					} else {
						if (bType.equals("java.util.Date"))
							continue;
						p = p + 1;
		...
		sn(sb, "    public static List<%s> getByPage(int page, int size) {", tbUEn);
		sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
		sn(sb, "        return getByPage(DAO, page, size, DAO.TABLENAME);");
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getByPage(%sDAO DAO, int page, int size) {", tbUEn, tbUEn);
		sn(sb, "        return getByPage(DAO, page, size, DAO.TABLENAME);");
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getByPage(int page, int size, String TABLENAME2) {", tbUEn);
		sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
		sn(sb, "        return getByPage(DAO, page, size, TABLENAME2);");
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getByPage(final %sDAO DAO, final int page, final int size,final String TABLENAME2) {", tbUEn, tbUEn);
		...
		sn(sb, "    public static int pageCount(final %sDAO DAO, final int size,final String TABLENAME2) {", tbUEn, tbUEn);
		sn(sb, "        int v = 0;");
		sn(sb, "        if( cacheModel == NO_CACHE ){");
		...
		while (it.hasNext()) {
			...
			if (idx_size == 1) { // 单索引
				...
				if (NON_UNIQUE.equals("false")) {
					sn(sb, "    public static %s getBy%s(%s %s) {", tbUEn, COLUMN_NAME_UEN, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
					sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static %s getBy%s(%sDAO DAO, %s %s) {", tbUEn, COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static %s getBy%s(%s %s, String TABLENAME2) {", tbUEn, COLUMN_NAME_UEN, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
					sn(sb, "        return getBy%s(DAO, %s, TABLENAME2);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static %s getBy%s(final %sDAO DAO, final %s %s,final String TABLENAME2) {", tbUEn, COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_BTYPE, COLUMN_NAME_EN);
					sn(sb, "        if( cacheModel == NO_CACHE ){");
					sn(sb, "            return DAO.selectBy%s(%s, TABLENAME2);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY");
					sn(sb, "            %s %s = varsBy%s.get(%s);", priKeyType, priKey, COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "            if(%s == null) return null;", priKey);
					sn(sb, "            %s result = getByKey(DAO, %s, TABLENAME2);", tbUEn, priKey);
					...
					sn(sb, "    public static int countBy%s(%sDAO DAO, %s %s) {", COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        return countBy%s(DAO, %s, DAO.TABLENAME);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");
					...
					sn(sb, "    public static int countBy%s(final %sDAO DAO, final %s %s,final String TABLENAME2) {", COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        if(cacheModel == NO_CACHE){");
					sn(sb, "            return DAO.countBy%s(%s, TABLENAME2);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "        }");
					sn(sb, "        List<%s> %ss = getBy%s(DAO, %s, TABLENAME2);", tbUEn, tbEn, COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "        return %ss.size();", tbEn);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static List<%s> getBy%s(%s %s) {", tbUEn, COLUMN_NAME_UEN, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
					sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static List<%s> getBy%s(%sDAO DAO, %s %s) {", tbUEn, COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static List<%s> getBy%s(%s %s, String TABLENAME2) {", tbUEn, COLUMN_NAME_UEN, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
					sn(sb, "        return getBy%s(DAO, %s, TABLENAME2);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static List<%s> getBy%s(final %sDAO DAO, final %s %s,final String TABLENAME2) {", tbUEn, COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        if( cacheModel == NO_CACHE ){");
					sn(sb, "            return DAO.selectBy%s(%s, TABLENAME2);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "        } else { //FULL_CACHE || FULL_MEMORY {");
					sn(sb, "            List<%s> result = newList();", tbUEn);
					sn(sb, "            Set<%s> m1 = varsBy%s.get(%s);", priKeyType, COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "            if (m1 == null || m1.isEmpty()) return result;");
					sn(sb, "            List<%s> list = new ArrayList(m1);", priKeyType);
					sn(sb, "            for (%s key : list) {;", pkBType);
					sn(sb, "                %s e = getByKey(DAO, key, TABLENAME2);", tbUEn);
					sn(sb, "                if(e == null){");
					sn(sb, "                    m1.remove(key); ");
					sn(sb, "                    continue;");
					sn(sb, "                }");
				...
				if (COLUMN_NAME_TYPE.equals("String")) {
					...
					sn(sb, "    public static int countLike%s(%sDAO DAO, %s %s) {", COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        return countLike%s(DAO, %s, DAO.TABLENAME);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");
					...
					sn(sb, "    public static int countLike%s(final %sDAO DAO, final %s %s,final String TABLENAME2) {", COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        if(cacheModel == NO_CACHE){");
					sn(sb, "            return DAO.countLike%s(%s, TABLENAME2);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "        }");
					sn(sb, "        List<%s> %ss = getLike%s(DAO, %s, TABLENAME2);", tbUEn, tbEn, COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "        return %ss.size();", tbEn);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static List<%s> getLike%s(%s %s) {", tbUEn, COLUMN_NAME_UEN, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
					sn(sb, "        return getLike%s(DAO, %s, DAO.TABLENAME);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static List<%s> getLike%s(%sDAO DAO, %s %s) {", tbUEn, COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        return getLike%s(DAO, %s, DAO.TABLENAME);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static List<%s> getLike%s(%s %s, String TABLENAME2) {", tbUEn, COLUMN_NAME_UEN, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
					sn(sb, "        return getLike%s(DAO, %s, TABLENAME2);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static List<%s> getLike%s(final %sDAO DAO, final %s %s,final String TABLENAME2) {", tbUEn, COLUMN_NAME_UEN, tbUEn, COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        if(cacheModel == NO_CACHE){");
					sn(sb, "            return DAO.selectLike%s(%s, TABLENAME2);", COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY");
					sn(sb, "            List<%s> result = newList();", tbUEn);
					sn(sb, "            List<%s> %ss = getAll(DAO, TABLENAME2);", tbUEn, tbEn);
					sn(sb, "            for(%s e : %ss){", tbUEn, tbEn);
					sn(sb, "                %s _var = e.get%s();", COLUMN_NAME_TYPE, COLUMN_NAME_UEN);
					sn(sb, "                if(_var.indexOf(%s) >= 0)", COLUMN_NAME_EN, priKey);
					sn(sb, "                    result.add(e);");
					sn(sb, "            }");
					// sn(sb, " sort(result);");
					sn(sb, "            return result;");
				...
				if (NON_UNIQUE.equals("false")) { // 唯一数据
					sn(sb, "    public static %s getBy%s(%s) {", tbUEn, index1, index2);
					sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
					sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);", index1, index3);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static %s getBy%s(%sDAO DAO, %s) {", tbUEn, index1, tbUEn, index2);
					sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);", index1, index3);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static %s getBy%s(%s, String TABLENAME2) {", tbUEn, index1, index2);
					sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
					sn(sb, "        return getBy%s(DAO, %s, TABLENAME2);", index1, index3);
					sn(sb, "    }");
					sn(sb, "");

					sn(sb, "    public static %s getBy%s(final %sDAO DAO, %s,final String TABLENAME2) {", tbUEn, index1, tbUEn, index2);
					sn(sb, "        if( cacheModel == NO_CACHE ){");
					sn(sb, "            return DAO.selectBy%s(%s, TABLENAME2);", index1, index3);
					sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY");
					sn(sb, "            String vkey = %s;", index5);
					sn(sb, "            %s %s = varsBy%s.get(vkey);", priKeyType, priKey, index1);
					sn(sb, "            if(%s == null) return null;", priKey);
					sn(sb, "            %s result = getByKey(DAO, %s, TABLENAME2);", tbUEn, priKey);
					sn(sb, "            if(result == null) return null;");
					{
				...
				} else { // 非唯一数据
					{
						...
						sn(sb, "    public static int countBy%s(%sDAO DAO, %s) {", index1, tbUEn, index2);
						sn(sb, "        return countBy%s(DAO, %s, DAO.TABLENAME);", index1, index3);
						sn(sb, "    }");
						sn(sb, "");
						...
						sn(sb, "    public static int countBy%s(final %sDAO DAO, %s, final String TABLENAME2) {", index1, tbUEn, index2);
						sn(sb, "        if(cacheModel == NO_CACHE){");
						sn(sb, "            return DAO.countBy%s(%s, TABLENAME2);", index1, index3);
						sn(sb, "        }");
						sn(sb, "        List<%s> %ss = getBy%s(%s, TABLENAME2);", tbUEn, tbEn, index1, index3);
						sn(sb, "        return %ss.size();", tbEn);
						sn(sb, "    }");
						sn(sb, "");
					}
					{
						sn(sb, "    public static List<%s> getBy%s(%s) {", tbUEn, index1, index2);
						sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
						sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);", index1, index3);
						sn(sb, "    }");
						sn(sb, "");

						sn(sb, "    public static List<%s> getBy%s(%sDAO DAO, %s) {", tbUEn, index1, tbUEn, index2);
						sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);", index1, index3);
						sn(sb, "    }");
						sn(sb, "");

						sn(sb, "    public static List<%s> getBy%s(%s, String TABLENAME2) {", tbUEn, index1, index2);
						sn(sb, "        %sDAO DAO = (cacheModel == NO_CACHE) ? DAO() : null;", tbUEn);
						sn(sb, "        return getBy%s(DAO, %s, TABLENAME2);", index1, index3);
						sn(sb, "    }");
						sn(sb, "");

						sn(sb, "    public static List<%s> getBy%s(final %sDAO DAO, %s, final String TABLENAME2) {", tbUEn, index1, tbUEn, index2);
						sn(sb, "        if( cacheModel == NO_CACHE ){");
						sn(sb, "            return DAO.selectBy%s(%s, TABLENAME2);", index1, index3);
						sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY");
						sn(sb, "            List<%s> result = newList();", tbUEn);
						sn(sb, "            String vkey = %s;", index5);
						sn(sb, "            Set<%s> m1 = varsBy%s.get(vkey);", priKeyType, index1);
						sn(sb, "            if (m1 == null || m1.isEmpty()) return result;");
						sn(sb, "            List<%s> list = new ArrayList(m1);", priKeyType);
						sn(sb, "            for (%s key : list) {", pkBType);
						sn(sb, "                %s e = getByKey(DAO, key, TABLENAME2);", tbUEn);
						...
						for (Map<String, Object> map : idx) {
							...
							String _COLUMN_NAME_UEN = StrEx.upperN1(_COLUMN_NAME_EN);
							String _COLUMN_NAME_TYPE = JTypeMysql.getType(rsmd, _COLUMN_NAME);
							String _basicType = JTypeMysql.getBasicType(_COLUMN_NAME_TYPE);
		...
		sn(sb, "    public static %s update(%sDAO DAO, %s %s) {", tbUEn, tbUEn, tbUEn, tbEn);
		sn(sb, "        return update(DAO, %s, DAO.TABLENAME);", tbEn);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s update(%s %s, String TABLENAME2) {", tbUEn, tbUEn, tbEn);
		sn(sb, "        %sDAO DAO = DAO();", tbUEn);
		sn(sb, "        return update(DAO, %s, TABLENAME2);", tbEn);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s update(final %sDAO DAO, final %s %s,final String TABLENAME2) {", tbUEn, tbUEn, tbUEn, tbEn);
		sn(sb, "        if(cacheModel != NO_CACHE){");
		...
		sn(sb, "    public static %s asyncUpdate(%sDAO DAO, %s %s) {", tbUEn, tbUEn, tbUEn, tbEn);
		sn(sb, "        return asyncUpdate(DAO, %s, DAO.TABLENAME);", tbEn);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s asyncUpdate(%s %s, String TABLENAME2) {", tbUEn, tbUEn, tbEn);
		sn(sb, "        %sDAO DAO = DAO();", tbUEn);
		sn(sb, "        return asyncUpdate(DAO, %s, TABLENAME2);", tbEn);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s asyncUpdate(final %sDAO DAO, final %s %s,final String TABLENAME2) {", tbUEn, tbUEn, tbUEn, tbEn);
		sn(sb, "        if(cacheModel != NO_CACHE){");
		sn(sb, "            put(%s, false);", tbEn);
		sn(sb, "        }");
		sn(sb, "        if(cacheModel != FULL_MEMORY){");
		...
		sn(sb, "    public static int inMemFlush(final %sDAO DAO, final String TABLENAME2) {", tbUEn);
		sn(sb, "        int result = 0;");
		sn(sb, "        if(cacheModel != FULL_MEMORY)");
		sn(sb, "            return result;");
		sn(sb, "        try {");
