...
public class DaoBuilder extends ExToolkit {
	...
	public static String build(Connection conn, ResultSet rs, String pkg, boolean batch, String db, String TABLENAME) throws Exception {
		...
		while (it.hasNext()) {
			...
			if (idx_size == 1) { // 单索引
				...
				if (NON_UNIQUE.equals("0")) {
					...
					sn(sb, "        try{");
					sn(sb, "            String sql=\"SELECT %s FROM ( SELECT ROWNUM r, a.* FROM \" + TABLENAME2 + \" a WHERE %s = :%s ORDER BY %s) b WHERE b.r >= \" + begin + \" AND b.r < \" + (begin + num) + \"\";",
					...
					// SELECT * FROM ( SELECT ROWNUM r, a.* FROM 表名 a WHERE
					// ROMNUM < m + n AND 条件 ORDER BY id ) b WHERE b.r > m
					sn(sb, "    public List<%s> selectPageBy%sPKs(final %s %s, final int begin, final int num, final String TABLENAME2) {", primaryKeyType, COLUMN_NAME_UEN, COLUMN_NAME_TYPE,
							COLUMN_NAME_EN);
					sn(sb, "        try{");
					sn(sb, "            List<%s> result = newList();", primaryKeyType);
					sn(sb, "            String sql=\"SELECT %s FROM ( SELECT ROWNUM r, a.* \" + TABLENAME2 + \" a WHERE ROWNUM < \" + (begin + num) + \" AND %s = :%s ORDER BY %s) b WHERE b.r > \" + begin + \"\";",
				...
				if (NON_UNIQUE.equals("0")) { // 唯一数据
					...
					sn(sb, "        try{");
					sn(sb, "            String sql=\"SELECT %s FROM ( SELECT ROWNUM r, a.* FROM \" + TABLENAME2 + \" a WHERE ROWNUM < \" + ( begin + num )+ \" AND %s ORDER BY %s ) b WHERE b.r > \" + begin + \"\";",
					...
					sn(sb, "        try{");
					sn(sb, "            List<%s> result = newList();", primaryKeyType);
					sn(sb, "            String sql=\"SELECT %s FROM ( SELECT ROWNUM r, a.* FROM \" + TABLENAME2 + \" a WHERE ROWNUM < \" + ( begin + num ) + \"%s ORDER BY %s ) WHERE b.r >  \" + begin + \"\";",
		...
		sn(sb, "        try{");
		sn(sb, "            String sql = \"SELECT %s FROM ( SELECT ROWNUM r, a.* FROM \" + TABLENAME2 + \" a ORDER BY %s ) b WHERE b.r >= \" + begin + \" AND b.r < \" + ( begin + num )+ \"\";",
				columns3, primaryKey);
		sn(sb, "            return super.queryForList(sql, %s.class);", tableUEn);
		sn(sb, "        } catch(Exception e) {");
		sn(sb, "            log.info(e2s(e));");
		...
		for (Map<String, Object> m : columns) {
			...
			if (!column.equals(primaryKey)) {
				if (bType.contains("short") || bType.contains("int") || bType.contains("long") || bType.contains("float") || bType.contains("double")) {
					...
					sn(sb, "        try{");
					sn(sb, "            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s = (select case when %s+:%s<=:_min then :_min else %s+:%s end) WHERE %s=:%s\";", column, column, column,
					...
					if (batch) {
						...
						sn(sb, "            }");
						sn(sb, "            String str = sb.toString();");
						sn(sb, "            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s = (select case when %s+:%s<=:_min then :_min else %s+:%s end) WHERE %s in (\" + str + \")\";", column,
								column, column, column, column, primaryKey);
						sn(sb, "            Map params = newMap();");
						sn(sb, "            params.put(\"_min\", _min);");
						sn(sb, "            params.put(\"%s\", %s);", column, columnEn);
						sn(sb, "            return super.update(sql, params);");
					...
					sn(sb, "        try{");
					sn(sb, "            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s = (select case when %s+:%s>=:_max then :_max else %s+:%s end) WHERE %s=:%s\";", column, column, column,
							column, column, primaryKey, primaryKey);
					sn(sb, "            Map params = newMap();");
					sn(sb, "            params.put(\"%s\", %s);", primaryKey, primaryKey);
					...
					if (batch) {
						...
						sn(sb, "            }");
						sn(sb, "            String str = sb.toString();");
						sn(sb, "            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s = (select case when %s+:%s>=:_max then :_max else %s+:%s end) WHERE %s in (\" + str + \")\";", column,
								column, column, column, column, primaryKey);
						sn(sb, "            Map params = newMap();");
						sn(sb, "            params.put(\"_max\", _max);");
						sn(sb, "            params.put(\"%s\", %s);", column, columnEn);
					...
					sn(sb, "    public int update%sWithMinMaxByKey(final %s %s, final %s %s, final %s _min, final %s _max, final String TABLENAME2){", columnUEn, pkBasicType, primaryKey, bType,
					...
					if (batch) {
						...
						sn(sb, "    public int update%sWithMinMaxInKeys(final List<%s> keys, final %s %s, final %s _min, final %s _max, final String TABLENAME2){", columnUEn, primaryKeyType, bType,
								columnEn, bType, bType);
						sn(sb, "        if( %s < 0 ) {", columnEn);
