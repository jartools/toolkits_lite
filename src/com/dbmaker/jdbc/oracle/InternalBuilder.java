...
public class InternalBuilder extends ExToolkit {
	...
	public static String build(Connection conn, ResultSet rs, String pkg, String appContext, boolean batch, String db, String TABLENAME) throws Exception {
		...
		//
		// sn(sb,
		// " public static int delete2(final %sDAO DAO, final %s %s,final String
		// TABLENAME2) {",
		...
		while (it.hasNext()) {
			...
			if (idx_size == 1) { // 单索引
				...
				String COLUMN_NAME_UEN = StrEx.upperFirst(COLUMN_NAME_EN);
				String COLUMN_NAME_TYPE = JTypeOracle.getType(rsmd, COLUMN_NAME);
				String COLUMN_NAME_BTYPE = JTypeOracle.getBasicType(COLUMN_NAME_TYPE);
				if (INDEX_NAME.equals("PRIMARY"))
					continue;
				...
				if (NON_UNIQUE.equals("0")) { // 唯一数据
					...
					}
					{
						...
						for (Map<String, Object> map : idx) {
							...
							String _COLUMN_NAME_TYPE = JTypeOracle.getType(rsmd, _COLUMN_NAME);
							String _basicType = JTypeOracle.getBasicType(_COLUMN_NAME_TYPE);
		...
		//
		// sn(sb,
		// " public static %s update2(final %sDAO DAO, final %s %s,final String
		// TABLENAME2) {",
		// tableUEn, tableUEn, tableUEn, tableEn);
		// sn(sb, " if(cacheModel != FULL_MEMORY){");
		// sn(sb, " execute(new Runnable() {");
