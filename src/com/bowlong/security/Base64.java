...
public class Base64 {
	 ...
	 * The method is about twice as fast as {@link #decode2Bytes(String)}. The
	 * preconditions are:<br>
	 * + The array must have a line length of 76 chars OR no line separators at all
	 * (one line).<br>
	 * + Line separator must be "\r\n", as specified in RFC 2045 + The array must
	...
	static final public byte[] toBytes(String src) {
		if (src == null)
			return null;
		try {
	...
	static final public String toStr(byte[] buff) {
		try {
	...
	static final public String decode(String s) {
		return toStr(decodeFast(s));
	}

	static final public String decode4Cs(String strCs64) {
		byte[] buf = DatatypeConverter.parseBase64Binary(strCs64);
		return toStr(buf);
	}

	static final public String encode4Cs(String strSrc) {
		byte[] buf = toBytes(strSrc);
		if (buf == null)
			return "";
