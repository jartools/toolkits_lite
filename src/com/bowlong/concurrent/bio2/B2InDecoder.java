...
public class B2InDecoder {
	...
	public static final byte read(ByteBuffer is) {
		return is.get();
	}

	public static final int readInt(ByteBuffer is) throws IOException {
		byte tag = read(is);
		switch (tag) {
		case B2Type.INT_N1:
			return -1;
		...
		case B2Type.INT_16B: {
			short v = (short) (((read(is) & 0xff) << 8) + ((read(is) & 0xff) << 0));
			return v;
		}
		case B2Type.INT_32B: {
			int value1 = read(is);
	...
	public static final Object readObject(ByteBuffer is) throws Exception {
		byte tag = read(is);
		switch (tag) {
		case B2Type.NULL: {
			return null;
		...
		case B2Type.LONG_16B: {
			int v = (((read(is) & 0xff) << 8) + ((read(is) & 0xff) << 0));
			return new Long(v);
		}
