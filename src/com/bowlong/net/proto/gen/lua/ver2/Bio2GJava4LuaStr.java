...
public class Bio2GJava4LuaStr {
	...
	// 向客户端主动发送
	static private void g2s_service_to_client(List<Method> methods,
			StrBuilder sb) {
		for (Method m : methods) {
			if (!B2G.isClient(m))
				continue;
			...
			for (NewMap<String, String> m1 : params) {
				...
				if (B2G.getMapType(key).equals("getObject")) {
					sb.pn("        _params.put(\"${1}\", ${2}.toMap());", hval,
							val);
				} else {
					if (B2G.getMapType(key).equals("getList")) {
						sb.pn("		{");
	...
	static public final List<Method> sortMethod(Method[] methods) {
		...
		Collections.sort(list, new Comparator<Method>() {
			...
			public int compare(Method o1, Method o2) {
				...
				int v = TkitBase.compareTo(oname1, oname2);
				if (v < 0) {
					return -1;
				} else if (v > 0) {
					return 1;
