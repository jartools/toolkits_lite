...
public class Bio2GJava4LuaStr extends ExToolkit {
	...
	public static void g2s_service(Class<?> c, String namespace, StrBuilder sb) {
		...
		List<Method> methods = sortMethod(arrmethods);
		...
		sb.pn("        String cmd = map.getString(\"cmd\");", B2G.METHOD);
		sb.pn("        return disp(chn, cmd, map);");
		sb.pn("    }");
		sb.pn("    public String disp(TcpChannel chn, String cmd, NewMap map) throws Exception {");
		sb.pn("        if(chn == null) return \"\";");
		sb.pn("        switch (cmd) {");
		for (Method m : methods) {
		...
		for (Method m : methods) {
			...
			if (B2G.isClient(m)) {
				...
				if (srtype.equals("void")) {
					...
					sb.pn("            writeMapEntry(result, \"${1}\", \"${2}\");", "cmd", hmname);
	...
	// 向客户端主动发送
	static private void g2s_service_to_client(List<Method> methods, StrBuilder sb) {
		for (Method m : methods) {
			if (!B2G.isClient(m))
				continue;
			String remark = B2G.getRemark(m);
			...
			sb.pn("        _params.put(\"cmd\", \"${2}\"); // cmd:${3}", B2G.METHOD, hmname, mname);
	...
	private static void g2s_pv(List<Method> methods, StrBuilder sb) {
		sb.pn("");
		sb.pn("");
		sb.pn("    // //////////////////////////////////////////////");
		sb.pn("    // PV  请求  参数 逻辑分发  ");
		...
		sb.pn("    }");
		sb.pn("    public String pv_params(String cmd, NewMap map) throws Exception {");
		sb.pn("        switch (cmd) {");
		for (Method m : methods) {
			String remark = B2G.getRemark(m);
			// String srtype = B2G.getReturnType(m);
	...
	public static void g2s_call(Class<?> c, String namespace, StrBuilder sb) {
		...
		for (Method m : methods) {
			...
			// String srtype = B2G.getReturnType(m);
			String mname = B2G.getNethodName(m);
			String hhmname = mname; // mname.hashCode() + "";
			NewList<NewMap<String, String>> params = B2G.getParameters(m);
			...
			for (NewMap<String, String> m1 : params) {
				...
				// String hval = myvar; // myvar.hashCode() + "";
		...
		sb.pn("        String cmd = map.getString(\"${1}\");", B2G.METHOD);
		sb.pn("        disp(cmd, map);");
		sb.pn("    }");
		sb.pn("    public void disp(String cmd, NewMap map) throws Exception {");
		sb.pn("        switch (cmd) {");
		for (Method m : methods) {
			String remark = B2G.getRemark(m);
			String srtype = B2G.getReturnType(m);
	...
	static public final List<Method> sortMethod(Method[] methods) {
		...
		Collections.sort(list, new Comparator<Method>() {
			...
			public int compare(Method o1, Method o2) {
				String oname1 = B2G.getNethodName(o1);
				String oname2 = B2G.getNethodName(o2);
				int v = compareTo(oname1, oname2);
				if (v < 0) {
					return -1;
				} else if (v > 0) {
					return 1;
