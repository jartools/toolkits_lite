...
public class DataSet extends JdbcTemplate {
	...
	public final int pageCount(final String c, final int size) throws SQLException {
		int count = count(c);
	...
	public final <T> T queryForObject(final String c, final Class c2) throws Exception {
	...
	public final <T> List<T> queryForList(final String c, final Class c2) throws Exception {
		StringBuffer sb = StringBufPool.borrowObject();
		try {
			sb.append("SELECT * FROM ").append(TABLENAME);
			if (c != null && !c.isEmpty()) {
	...
	public final List<Map> queryForList(final String c, final int begin, final int num)
			throws SQLException {
		StringBuffer sb = StringBufPool.borrowObject();
		try {
			sb.append("SELECT * FROM ").append(TABLENAME);
	...
	public final <T> List<T> queryForList(final String c, final Class c2, final int begin, final int num)
			throws Exception {
		StringBuffer sb = StringBufPool.borrowObject();
		try {
			sb.append("SELECT * FROM ").append(TABLENAME);
	...
	public final List<Map> queryForList(final int begin, final int num) throws SQLException {
		return queryForList("", begin, num);
	}

	public final <T> List<T> queryForList(final Class c2, final int begin, final int num)
			throws Exception {
	...
	public final int update(final BeanSupport x, final String c) throws SQLException {
		return update(x.toBasicMap(), c);
	}

	public final int update(final Map<String, Object> m, final String c) throws SQLException {
		StringBuffer sb = StringBufPool.borrowObject();
		try {
			List<String> keys = newList();
			keys.addAll(m.keySet());
	...
	public final int delete(final Map<String, Object> m, final String c) throws SQLException {
		StringBuffer sb = StringBufPool.borrowObject();
		try {
	...
	public final int delete(final BeanSupport x, final String c) throws SQLException {
