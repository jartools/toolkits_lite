...
public class InternalBuilder extends Toolkit {
	public static void main(String[] args) throws Exception {
		...
		try (Connection conn = SqlEx.newOracleConnection(host, port, db, user,
			...
			String xml = build(conn, rs, bpackage, appContext, batch, db,
					TABLENAME);
	...
	public static String build(Connection conn, ResultSet rs, String pkg,
			String appContext, boolean batch, String db, String TABLENAME)
			throws Exception {
		...
		// primaryKeyType);
		sn(sb, "");
		sn(sb, "    public static final Map<%s, %s> vars = newMap();",
				primaryKeyType, tableUEn);
		{
			...
			while (it.hasNext()) {
				...
				if (idx_size == 1) { // 单索引
					...
					if (NON_UNIQUE.equals("0")) { // 唯一数据
						sn(sb,
								"    public static final Map<%s, %s> varsBy%s = newMap();",
								COLUMN_NAME_TYPE, primaryKeyType,
								COLUMN_NAME_UEN);
					} else {
						if (COLUMN_NAME_TYPE.equals("java.util.Date"))
							continue;
						sn(sb,
								"    public static final Map<%s, Map<%s, %s>> varsBy%s = newMap();",
								COLUMN_NAME_TYPE, primaryKeyType,
								primaryKeyType, COLUMN_NAME_UEN);
					}
					...
					if (NON_UNIQUE.equals("0")) { // 唯一数据
						sn(sb,
								"    public static final Map<String, %s> varsBy%s = newMap();",
								primaryKeyType, index1);
					} else {
						sn(sb,
								"    public static final Map<String, Map<%s, %s>> varsBy%s = newMap();",
								primaryKeyType, primaryKeyType, index1);
					}
				}
			}
		}
		...
		// sn(sb,
		// "    public static boolean isTimeout(long TIMEOUT, long LASTTIME) {");
		...
		// sn(sb,
		// "    public static boolean isTimeout(long TIMEOUT, Date DLASTTIME) {");
		...
		sn(sb, "        %s %s = %s.%s;", pkBasicType, primaryKey, tableEn,
				primaryKey);
			...
			while (it.hasNext()) {
				...
				if (idx_size == 1) { // 单索引
					...
					if (INDEX_NAME.equals("PRIMARY")
						...
						continue;
					if (NON_UNIQUE.equals("0")) { // 唯一数据
						sn(sb, "        %s %s = %s.get%s();", basicType,
								COLUMN_NAME_EN, tableEn, COLUMN_NAME_UEN);
						sn(sb, "        varsBy%s.put(%s, %s);",
								COLUMN_NAME_UEN, COLUMN_NAME_EN, primaryKey);
					} else {
						...
						sn(sb, "        %s %s = %s.get%s();", basicType,
								COLUMN_NAME_EN, tableEn, COLUMN_NAME_UEN);
						sn(sb, "        Map m%d = varsBy%s.get(%s);", p,
								COLUMN_NAME_UEN, COLUMN_NAME_EN);
						sn(sb, "        if(m%d == null){", p);
						sn(sb, "            m%d = newMap();", p);
						sn(sb, "            varsBy%s.put(%s, m%d);",
								COLUMN_NAME_UEN, COLUMN_NAME_EN, p);
						sn(sb, "        }", p);
						sn(sb, "        m%d.put(%s, %s);", p, primaryKey,
								primaryKey);
					}
					sn(sb, "");
					...
					if (NON_UNIQUE.equals("0")) { // 唯一数据
						...
						for (Map<String, Object> map : idx) {
							...
							sn(sb, "            %s v%s = %s.get%s();",
									basicType, COLUMN_NAME_EN, tableEn,
									COLUMN_NAME_UEN);
						...
						sn(sb, "            String vkey = %s;", skey);
						sn(sb, "            Map m%d = varsBy%s.get(vkey);", p,
								index1);
						sn(sb, "            if(m%d == null){", p);
						sn(sb, "                m%d = newMap();", p);
						...
						sn(sb, "            m%d.put(%s, %s);", p, primaryKey,
								primaryKey);
						// sn(sb, "        varsBy%s.put(_key, %s);", index1,
						// primaryKey);
						sn(sb, "        }");
					}
					sn(sb, "");
		...
		// sn(sb, "");
		//
		// sn(sb, "    public static String createTableYy(%sDAO DAO) {",
		// tableUEn);
		...
		// sn(sb, "");
		//
		// sn(sb, "    public static String createTableMm(%sDAO DAO) {",
		// tableUEn);
		...
		// sn(sb, "    public static String createTableDd(%sDAO DAO) {",
		// tableUEn);
		...
		// sn(sb, "");
		//
		// sn(sb, "    public static void createNoUniqueTable(%sDAO DAO) {",
		// tableUEn);
		...
		sn(sb, "    public static List<Map> toMap(List<%s> %ss){", tableUEn,
				tableEn);
		...
		sn(sb, "    public static List<%s> sort(List<%s> %ss){", tableUEn,
				tableUEn, tableEn);
		sn(sb, "        Collections.sort(%ss, new Comparator<%s>(){", tableEn,
				tableUEn);
		sn(sb, "            public int compare(%s o1, %s o2) {", tableUEn,
				tableUEn);
		...
		sn(sb, "    public static List<%s> sortReverse(List<%s> %ss){",
			...
			while (it.hasNext()) {
				...
				if (idx_size == 1) { // 单索引
					...
					sn(sb, "            public int compare(%s o1, %s o2) {",
							tableUEn, tableUEn);
					if (basicType.equals("int")) {
						sn(sb, "                %s v1 = (int)o1.get%s();",
								basicType, COLUMN_NAME_UEN);
						sn(sb, "                %s v2 = (int)o2.get%s();",
								basicType, COLUMN_NAME_UEN);
						sn(sb, "                return v1 - v2;");
					} else if (basicType.contains("BigDecimal")) {
						sn(sb,
								"                %s v1 = o1.get%s().intValue();",
								basicType, COLUMN_NAME_UEN);
						sn(sb,
								"                %s v2 = o2.get%s().intValue();",
								basicType, COLUMN_NAME_UEN);
						sn(sb, "                return v1 - v2;");
		...
		sn(sb, "    public static %s insert(%s %s) {", tableUEn, tableUEn,
				tableEn);
		sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		sn(sb, "    public static %s insert(%sDAO DAO, %s %s) {", tableUEn,
				tableUEn, tableUEn, tableEn);
		sn(sb, "        return insert(DAO, %s, DAO.TABLENAME);", tableEn);
		sn(sb, "    }");
		sn(sb, "");
		...
		sn(sb,
				...
				tableUEn, tableUEn, tableUEn, tableEn);
		sn(sb, "        int n = 0;");
		...
		// sn(sb, "    public static %s insert2(%s %s) {", tableUEn, tableUEn,
		// tableEn);
		// sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		// sn(sb, "");
		//
		// sn(sb, "    public static %s insert2(%sDAO DAO, %s %s) {", tableUEn,
		// tableUEn, tableUEn, tableEn);
		// sn(sb, "        return insert2(DAO, %s, DAO.TABLENAME);", tableEn);
		// sn(sb, "    }");
		// sn(sb, "");
		...
		// sn(sb, "        }");
		// sn(sb, "");
		// sn(sb, "        %s.%s = new %s(n);", tableEn, primaryKey,
		// pkBasicType);
		// sn(sb, "        if(cacheModel != NO_CACHE) put(%s);", tableEn);
		// sn(sb, "");
		// sn(sb, "        return %s;", tableEn);
		...
		if (batch) { // 批处理
			sn(sb, "    public static int[] insert(List<%s> %ss) {", tableUEn,
					tableEn);
			sn(sb, "        %sDAO DAO = DAO();", tableUEn);
			...
			sn(sb, "        int n = 0;");
			sn(sb, "        for(%s %s : %ss){", tableUEn, tableEn, tableEn);
			sn(sb, "            %s = insert(DAO, %s, TABLENAME2);", tableEn,
					tableEn);
			sn(sb,
					"            ret[n++] = (%s == null) ? 0 : %s.%s.intValue();",
					tableEn, tableEn, primaryKey);
			sn(sb, "        }");
			sn(sb, "        return ret;");
			sn(sb, "    }");
			sn(sb, "");
		...
		sn(sb, "    public static int delete(%sDAO DAO, %s %s) {", tableUEn,
		...
		if (batch) { // 批处理
			sn(sb, "    public static int[] delete(%s[] %ss) {", pkBasicType,
					primaryKey);
			sn(sb, "        %sDAO DAO = DAO();", tableUEn);
			sn(sb, "        return delete(DAO, %ss, DAO.TABLENAME);",
					primaryKey);
			sn(sb, "    }");
			sn(sb, "");
			...
			sn(sb, "        int n = 0;");
			sn(sb, "        for(%s %s : %ss){", pkBasicType, primaryKey,
					primaryKey);
		...
		// sn(sb, "    public static int delete2(%s %s) {", pkBasicType,
		// primaryKey);
		// sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		// sn(sb, "        return delete2(DAO, %s, DAO.TABLENAME);",
		// primaryKey);
		// sn(sb, "    }");
		// sn(sb, "");
		//
		// sn(sb, "    public static int delete2(%sDAO DAO, %s %s) {", tableUEn,
		// pkBasicType, primaryKey);
		// sn(sb, "        return delete2(DAO, %s, DAO.TABLENAME);",
		// primaryKey);
		// sn(sb, "    }");
		// sn(sb, "");
		//
		// sn(sb, "    public static int delete2(%s %s, String TABLENAME2) {",
		// pkBasicType, primaryKey);
		// sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		//
		// sn(sb,
		// "    public static int delete2(final %sDAO DAO, final %s %s,final String TABLENAME2) {",
		// tableUEn, pkBasicType, primaryKey);
		// sn(sb, "        if(cacheModel != FULL_MEMORY)");
		// sn(sb, "            execute(new Runnable() {");
		// sn(sb, "                public void run() {");
		// sn(sb, "                    DAO.deleteByKey(%s, TABLENAME2);",
		// primaryKey);
		// sn(sb, "                }");
		// sn(sb, "            });");
		// sn(sb, "        if(cacheModel != NO_CACHE) {");
		...
		sn(sb, "    public static List<%s> getAll(%sDAO DAO) {", tableUEn,
				tableUEn);
		sn(sb, "        return getAll(DAO, DAO.TABLENAME);", primaryKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getAll(String TABLENAME2) {",
				tableUEn);
		sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		sn(sb, "    public static List<%s> getNoSortAll(%sDAO DAO) {",
				tableUEn, tableUEn);
		sn(sb, "        return getNoSortAll(DAO, DAO.TABLENAME);", primaryKey);
		sn(sb, "    }");
		sn(sb, "");
		...
		sn(sb, "    public static List<%s> getPks(%sDAO DAO) {",
				primaryKeyType, tableUEn);
		sn(sb, "        return getPks(DAO, DAO.TABLENAME);", primaryKey);
		sn(sb, "    }");
		sn(sb, "");
		...
		sn(sb, "    public static List<%s> getInIndex(%sDAO DAO) {", tableUEn,
				tableUEn);
		sn(sb, "        return getInIndex(DAO, DAO.TABLENAME);", primaryKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static List<%s> getInIndex(String TABLENAME2) {",
				tableUEn);
		sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		sn(sb,
				"    public static List<%s> getInIndex(final %sDAO DAO, final String TABLENAME2) {",
				tableUEn, tableUEn);
		...
		if (batch) {
			sn(sb, "    public static List<%s> getIn(List<%s> keys) {",
					tableUEn, primaryKeyType);
			sn(sb, "        %sDAO DAO = DAO();", tableUEn);
			sn(sb, "        return getIn(keys, DAO, DAO.TABLENAME);",
					primaryKey);
			sn(sb, "    }");
			sn(sb, "");
			...
			sn(sb,
					...
					tableUEn, primaryKeyType, tableUEn);
			sn(sb, "        return getIn(keys, DAO, DAO.TABLENAME);",
					primaryKey);
			sn(sb, "    }");
			sn(sb, "");
			...
			sn(sb,
					...
					tableUEn, primaryKeyType);
			sn(sb, "        %sDAO DAO = DAO();", tableUEn);
			sn(sb, "        return getNoSortIn(keys, DAO, TABLENAME2);",
					primaryKey);
			sn(sb, "    }");
			sn(sb, "");
			...
			sn(sb, "                %s %s = getByKeyFromMemory(key);",
					tableUEn, tableEn);
		...
		sn(sb, "            List<%s> mvars = getAll(DAO, TABLENAME2);",
				tableUEn);
		sn(sb, "            if( mvars.size() > num ){");
		sn(sb,
				"                result = mvars.subList(mvars.size() - num, mvars.size() - 1);");
		sn(sb, "            }");
		sn(sb, "        }");
		sn(sb, "        return result;");
		sn(sb, "    }");
		...
		sn(sb, "    public static List<%s> getGtKey(%s %s) {", tableUEn,
				pkBasicType, primaryKey);
		sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		sn(sb, "    public static List<%s> getGtKey(%sDAO DAO, %s %s) {",
				tableUEn, tableUEn, pkBasicType, primaryKey);
		sn(sb, "        return getGtKey(DAO, %s, DAO.TABLENAME);", primaryKey);
		sn(sb, "    }");
		sn(sb, "");
		...
		sn(sb, "        List<%s> result = newList();", tableUEn);
		sn(sb, "        if( cacheModel == NO_CACHE ){");
		sn(sb, "            return DAO.selectGtKey(%s, TABLENAME2);",
				primaryKey);
		sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY ");
		sn(sb, "            List<%s> %ss = getAll();", tableUEn, tableEn);
		sn(sb, "            for (%s %s : %ss) {", tableUEn, tableEn, tableEn);
		sn(sb,
				"                if(%s.%s.intValue() <= %s.intValue()) continue;",
				tableEn, primaryKey, primaryKey);
		sn(sb, "                result.add(%s);", tableEn);
		sn(sb, "            }");
		sn(sb, "        }");
		// sn(sb, "        sort(result);");
		...
		sn(sb, "    public static %s getByKey(%s %s) {", tableUEn, pkBasicType,
				primaryKey);
		sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		sn(sb, "    public static %s getByKey(%sDAO DAO, %s %s) {", tableUEn,
				tableUEn, pkBasicType, primaryKey);
		sn(sb, "        return getByKey(DAO, %s, DAO.TABLENAME);", primaryKey);
		sn(sb, "    }");
		sn(sb, "");
		...
		sn(sb,
				...
				tableUEn, tableUEn, pkBasicType, primaryKey);
		sn(sb, "        if(cacheModel == NO_CACHE){");
		sn(sb, "            return DAO.selectByKey(%s, TABLENAME2);",
				primaryKey);
		sn(sb, "        }");
		...
		sn(sb, "    public static %s getByKeyFromMemory(final %s %s) {",
				tableUEn, pkBasicType, primaryKey);
		sn(sb, "        return vars.get(%s);", primaryKey);
		sn(sb, "    }");
		sn(sb, "");

		sn(sb, "    public static %s deleteFromMemory(final %s %s) {",
				tableUEn, pkBasicType, primaryKey);
		...
		sn(sb, "    public static int pageCount(%sDAO DAO, int size) {",
				tableUEn);
		sn(sb, "        return pageCount(DAO, size, DAO.TABLENAME);");
		sn(sb, "    }");
		sn(sb, "");
		...
		while (it.hasNext()) {
			...
			if (idx_size == 1) { // 单索引
				...
				if (NON_UNIQUE.equals("0")) {
					sn(sb, "    public static %s getBy%s(%s %s) {", tableUEn,
					...
					sn(sb, "    public static %s getBy%s(%sDAO DAO, %s %s) {",
							tableUEn, COLUMN_NAME_UEN, tableUEn,
							COLUMN_NAME_TYPE, COLUMN_NAME_EN);
					sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);",
							COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					sn(sb, "");
					...
					sn(sb,
							...
							tableUEn, COLUMN_NAME_UEN, tableUEn,
							COLUMN_NAME_BTYPE, COLUMN_NAME_EN);
					sn(sb, "        if( cacheModel == NO_CACHE ){");
					sn(sb,
							"            return DAO.selectBy%s(%s, TABLENAME2);",
							COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY");
					sn(sb, "            %s %s = varsBy%s.get(%s);",
							primaryKeyType, primaryKey, COLUMN_NAME_UEN,
							COLUMN_NAME_EN);
					sn(sb, "            if(%s == null) return null;",
							primaryKey);
					sn(sb,
					...
					sn(sb,
							...
							COLUMN_NAME_UEN, tableUEn, COLUMN_NAME_TYPE,
							COLUMN_NAME_EN);
					sn(sb, "        return countBy%s(DAO, %s, DAO.TABLENAME);",
							COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "    }");
					...
					sn(sb,
							...
							tableUEn, tableEn, COLUMN_NAME_UEN, COLUMN_NAME_EN);
					...
					sn(sb,
							...
							COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb, "        } else { //FULL_CACHE || FULL_MEMORY {");
					sn(sb, "            Map m1 = varsBy%s.get(%s);",
							COLUMN_NAME_UEN, COLUMN_NAME_EN);
					sn(sb,
							"            if (m1 == null || m1.isEmpty()) return result;");
					sn(sb, "            List<%s> list = newList();",
							primaryKeyType);
				...
				if (NON_UNIQUE.equals("0")) { // 唯一数据
					sn(sb, "    public static %s getBy%s(%s) {", tableUEn,
							index1, index2);
					sn(sb, "        %sDAO DAO = DAO();", tableUEn);
					sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);",
							index1, index3);
					...
					sn(sb, "    public static %s getBy%s(%sDAO DAO, %s) {",
							tableUEn, index1, tableUEn, index2);
					sn(sb, "        return getBy%s(DAO, %s, DAO.TABLENAME);",
							index1, index3);
					sn(sb, "    }");
					sn(sb, "");
					...
					sn(sb, "            %s %s = varsBy%s.get(vkey);",
							primaryKeyType, primaryKey, index1);
					sn(sb, "            if(%s == null) return null;",
							primaryKey);
					sn(sb,
							"            %s result = getByKey(DAO, %s, TABLENAME2);",
				...
				} else { // 非唯一数据
					{
						...
						sn(sb,
								...
								tableUEn, tableEn, index1, index3);
						sn(sb, "        return %ss.size();", tableEn);
						sn(sb, "    }");
						sn(sb, "");
					}
					{
						...
						sn(sb, "        } else { // FULL_CACHE || FULL_MEMORY");
						sn(sb, "            String vkey = %s;", index5);
						sn(sb, "            Map m1 = varsBy%s.get(vkey);",
								index1);
						sn(sb,
								"            if (m1 == null || m1.isEmpty()) return result;");
						sn(sb, "            List<%s> list = newList();",
		...
		sn(sb, "    public static %s update(%s %s) {", tableUEn, tableUEn,
				tableEn);
		sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		sn(sb, "    public static %s update(%sDAO DAO, %s %s) {", tableUEn,
				tableUEn, tableUEn, tableEn);
		sn(sb, "        return update(DAO, %s, DAO.TABLENAME);", tableEn);
		sn(sb, "    }");
		sn(sb, "");
		...
		// sn(sb, "    public static %s update2(%s %s) {", tableUEn, tableUEn,
		// tableEn);
		// sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		// sn(sb, "");
		//
		// sn(sb, "    public static %s update2(%sDAO DAO, %s %s) {", tableUEn,
		// tableUEn, tableUEn, tableEn);
		// sn(sb, "        return update2(DAO, %s, DAO.TABLENAME);", tableEn);
		// sn(sb, "    }");
		// sn(sb, "");
		//
		// sn(sb, "    public static %s update2(%s %s, String TABLENAME2) {",
		// tableUEn, tableUEn, tableEn);
		// sn(sb, "        %sDAO DAO = DAO();", tableUEn);
		...
		//
		// sn(sb,
		// "    public static %s update2(final %sDAO DAO, final %s %s,final String TABLENAME2) {",
		// tableUEn, tableUEn, tableUEn, tableEn);
		// sn(sb, "        if(cacheModel != FULL_MEMORY){");
		// sn(sb, "            execute(new Runnable() {");
		// sn(sb, "                public void run() {");
		// sn(sb, "                    DAO.updateByKey(%s, TABLENAME2);",
		// tableEn);
		// sn(sb, "                }");
		// sn(sb, "            });");
		// sn(sb, "        }");
		...
		// sn(sb, "");
		//
		// sn(sb, "    public static int inMemFlush(String TABLENAME2) {",
		// tableUEn);
