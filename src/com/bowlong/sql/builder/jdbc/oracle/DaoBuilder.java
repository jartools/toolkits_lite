...
public class DaoBuilder extends Toolkit {
	...
	public static String build(Connection conn, ResultSet rs, String pkg,
			boolean batch, String db, String TABLENAME) throws Exception {
		...
		String table = TABLENAME.toUpperCase(); // (String)
												// columns.get(0).get("tableName");
		...
		// String createNoUniqueSql = SqlEx.createMysqlNoUniqueTable(conn, rs,
		// table);
		...
		sn(sb,
				"public class %sDAO extends com.bowlong.sql.oracle.JdbcTemplate {",
				tableUEn);
		...
		sn(sb,
				"    public %sDAO(javax.sql.DataSource ds_r, javax.sql.DataSource ds_w) {",
				tableUEn);
		...
		sn(sb, "        try {");
		sn(sb,
				"            String sql = \"INSERT INTO \" + TABLENAME2 + \" (%s) VALUES (%s)\";",
				columns4, columnsForInsert);
		sn(sb,
				"            String sqlId = \"SELECT %s FROM \" + TABLENAME2 + \" WHERE ROWID=?\";",
				primaryKey);
		...
		// "            String sqlId = \"SELECT %s FROM \" + TABLENAME2 + \" WHERE ROWID=?\";",
		// primaryKey);
		// sn(sb, "            Map map = super.insert(sql, sqlId,  %s);",
		// tableEn);
		// sn(sb, "            return getInt(map, \"%s\");", primaryKey);
		// sn(sb, "        } catch(Exception e) {");
		// sn(sb, "            log.info(e2s(e));");
		...
		if (batch) { // 批处理
			sn(sb, "    public int[] insert(final List<%s> %ss) {", tableUEn,
					tableEn);
		...
		sn(sb, "    public int deleteByKey(final %s %s) {", pkBasicType,
				primaryKey);
		...
		sn(sb, "        try{");
		sn(sb,
				"            String sql = \"DELETE FROM \" + TABLENAME2 + \" WHERE %s=:%s\";",
		...
		if (batch) { // 批处理
			sn(sb, "    public int[] deleteByKey(final %s[] %ss) {",
					pkBasicType, primaryKey);
			...
			sn(sb, "                Map params = newMap();");
			sn(sb, "                params.put(\"%s\", %s);", primaryKey,
					primaryKey);
		...
		if (batch) {
			...
			sn(sb,
					"            String sql = \"DELETE FROM \" + TABLENAME2 + \" WHERE %s in (\" + str + \" ) \";",
					primaryKey);
		...
		sn(sb, "        try{");
		sn(sb,
				"            String sql = \"SELECT %s FROM \" + TABLENAME2 + \" ORDER BY %s\";",
				columns3, primaryKey);
		sn(sb, "            return super.queryForList(sql, %s.class);",
				tableUEn);
		sn(sb, "        } catch(Exception e) {");
		sn(sb, "            log.info(e2s(e));");
		...
		if (ics != null && ics.length() >= 1) {
			...
			sn(sb,
					"    public List<%s> selectInIndex(final String TABLENAME2) {",
					tableUEn);
			sn(sb, "        try{");
			sn(sb,
					"            String sql = \"SELECT %s FROM \" + TABLENAME2 + \" ORDER BY %s\";",
		...
		if (batch) {
			...
			sn(sb, "            String str = sb.toString();");
			sn(sb,
					"            String sql = \"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s in (\" + str + \" ) ORDER BY %s\";",
					columns3, primaryKey, primaryKey);
			sn(sb, "            return super.queryForList(sql, %s.class);",
					tableUEn);
			sn(sb, "        } catch(Exception e) {");
			sn(sb, "            log.info(e2s(e));");
		...
		sn(sb, "        try{");
		sn(sb,
				"            String sql = \"SELECT %s FROM \" + TABLENAME2 + \" WHERE ROWNUM < \" + num + \" ORDER BY %s DESC \";",
				columns3, primaryKey);
		sn(sb, "            return super.queryForList(sql, %s.class);",
				tableUEn);
		sn(sb, "        } catch(Exception e) {");
		...
		sn(sb, "    public List<%s> selectLastPKs(final int num) {",
				primaryKeyType);
		...
		sn(sb, "            List<%s> result = newList();", primaryKeyType);
		sn(sb,
				"            String sql = \"SELECT %s FROM \" + TABLENAME2 + \" ROWNUM < \" + num + \" ORDER BY %s DESC \";",
				primaryKey, primaryKey);
		sn(sb, "            List<Map> dbresult = super.queryForList(sql);");
		sn(sb, "            for(Map map : dbresult){");
		sn(sb, "                result.add( (%s)map.get(\"%s\") );",
		...
		sn(sb, "        try{");
		sn(sb,
				"            String sql = \"SELECT %s FROM \" + TABLENAME2 + \" WHERE ROWNUM < 1 ORDER BY %s DESC \";",
				columns3, primaryKey);
		sn(sb, "            return super.queryForObject(sql, %s.class);",
				tableUEn);
		sn(sb, "        } catch(Exception e) {");
		sn(sb, "            // log.info(e2s(e));");
		sn(sb, "            return null;");
		...
		sn(sb, "        return selectGtKeyNum(%s, _num, TABLENAME);",
				primaryKey);
		sn(sb, "    }");
		sn(sb, "");
		...
		sn(sb, "        try{");
		sn(sb,
				"            String sql = \"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s > :%s AND ROWNUM < \" + _num + \"ORDER BY %s \";",
				columns3, primaryKey, primaryKey, primaryKey);
		sn(sb, "            Map params = newMap();");
		sn(sb, "            params.put(\"%s\", %s);", primaryKey, primaryKey);
		sn(sb, "            return super.queryForList(sql, params, %s.class);",
				tableUEn);
		sn(sb, "        } catch(Exception e) {");
		sn(sb, "            log.info(e2s(e));");
		sn(sb, "            return newList();");
		...
		sn(sb, "    public List<%s> selectGtKey(final %s %s) {", tableUEn,
				pkBasicType, primaryKey);
		...
		sn(sb, "        try{");
		sn(sb,
				"            String sql = \"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s > :%s ORDER BY %s\";",
				columns3, primaryKey, primaryKey, primaryKey);
		sn(sb, "            Map params = newMap();");
		sn(sb, "            params.put(\"%s\", %s);", primaryKey, primaryKey);
		sn(sb, "            return super.queryForList(sql, params, %s.class);",
		...
		sn(sb, "    public List<%s> selectGtKeyPKs(final %s %s) {",
				primaryKeyType, pkBasicType, primaryKey);
		...
		sn(sb, "            params.put(\"%s\", %s);", primaryKey, primaryKey);
		sn(sb,
				"            List<Map> dbresult = super.queryForList(sql, params);");
		sn(sb, "            for(Map map : dbresult){");
		sn(sb, "                result.add( (%s)map.get(\"%s\") );",
				primaryKeyType, primaryKey);
		sn(sb, "            }");
		...
		sn(sb, "    public %s selectByKey(final %s %s) {", tableUEn,
				pkBasicType, primaryKey);
		...
		sn(sb, "        try{");
		sn(sb,
				"            String sql = \"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s = :%s\";",
				columns3, primaryKey, primaryKey);
		sn(sb, "            Map params = newMap();");
		sn(sb, "            params.put(\"%s\", %s);", primaryKey, primaryKey);
		sn(sb,
				"            return super.queryForObject(sql, params, %s.class);",
				tableUEn);
		sn(sb, "        } catch(Exception e) {");
		sn(sb, "            // log.info(e2s(e));");
		sn(sb, "            return null;");
		...
		while (it.hasNext()) {
			...
			if (idx_size == 1) { // 单索引
				...
				if (NON_UNIQUE.equals("0")) {
					sn(sb, "    public %s selectBy%s(final %s %s) {", tableUEn,
					...
					sn(sb,
							"            String sql=\"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s = :%s\";",
							columns3, COLUMN_NAME, COLUMN_NAME);
					sn(sb, "            Map params = newMap();");
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql=\"SELECT COUNT(*) FROM \" + TABLENAME2 + \" WHERE %s = :%s \";",
					...
					sn(sb,
							"            String sql=\"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s = :%s ORDER BY %s \";",
							columns3, COLUMN_NAME, COLUMN_NAME, primaryKey);
					sn(sb, "            Map params = newMap();");
					sn(sb, "            params.put(\"%s\", %s);", COLUMN_NAME,
							COLUMN_NAME_EN);
					sn(sb,
							"            return super.queryForList(sql, params, %s.class);",
							tableUEn);
					sn(sb, "        } catch(Exception e) {");
					sn(sb, "            log.info(e2s(e));");
					sn(sb, "            return newList();");
					...
					sn(sb, "    public List<%s> selectBy%sPKs(final %s %s) {",
							primaryKeyType, COLUMN_NAME_UEN, COLUMN_NAME_TYPE,
							COLUMN_NAME_EN);
					...
					sn(sb,
							"            String sql=\"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s = :%s ORDER BY %s \";",
							primaryKey, COLUMN_NAME, COLUMN_NAME, primaryKey);
					sn(sb, "            Map params = newMap();");
					sn(sb, "            params.put(\"%s\", %s);", COLUMN_NAME,
							COLUMN_NAME_EN);
					sn(sb,
					...
					// SELECT * FROM ( SELECT ROWNUM r, a.* FROM 表名 a WHERE
					// ROMNUM < m + n AND 条件 ORDER BY id ) b WHERE b.r > m
					sn(sb,
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql=\"SELECT %s FROM ( SELECT ROWNUM r, a.* FROM \" + TABLENAME2 + \" a WHERE %s = :%s ORDER BY %s) b WHERE b.r >= \" + begin + \" AND b.r < \" + (begin + num) + \"\";",
							columns3, COLUMN_NAME, COLUMN_NAME, primaryKey);
					sn(sb, "            Map params = newMap();");
					sn(sb, "            params.put(\"%s\", %s);", COLUMN_NAME,
							COLUMN_NAME_EN);
					...
					sn(sb, "            List<%s> result = newList();",
							primaryKeyType);
					sn(sb,
							"            String sql=\"SELECT %s FROM ( SELECT ROWNUM r, a.* \" + TABLENAME2 + \" a WHERE ROWNUM < \" + (begin + num) + \" AND %s = :%s ORDER BY %s) b WHERE b.r > \" + begin + \"\";",
							primaryKey, COLUMN_NAME, COLUMN_NAME, primaryKey);
					sn(sb, "            Map params = newMap();");
					sn(sb, "            params.put(\"%s\", %s);", COLUMN_NAME,
							COLUMN_NAME_EN);
				...
				if (COLUMN_NAME_TYPE.equals("String")) {
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql=\"SELECT COUNT(*) FROM \" + TABLENAME2 + \" WHERE %s LIKE '%%\" + "
									+ COLUMN_NAME_EN + " + \"%%' \";",
							COLUMN_NAME);
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql=\"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s LIKE '%%\" + "
									+ COLUMN_NAME_EN
									+ " + \"%%' ORDER BY %s \";", columns3,
							COLUMN_NAME, primaryKey);
					sn(sb,
							"            return super.queryForList(sql, %s.class);",
							tableUEn);
					sn(sb, "        } catch(Exception e) {");
					sn(sb, "            log.info(e2s(e));");
					sn(sb, "            return newList();");
				...
				if (NON_UNIQUE.equals("0")) { // 唯一数据
					sn(sb, "    public %s selectBy%s(final %s) {", tableUEn,
							index1, index2);
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql=\"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s\";",
							columns3, index4);
					sn(sb, "            Map params = newMap();");
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql=\"SELECT COUNT(*) FROM \" + TABLENAME2 + \" WHERE %s \";",
							index4);
					sn(sb, "            Map params = newMap();");
					for (Map<String, Object> m : idx) {
						// String INDEX_NAME = MapEx.get(m, "INDEX_NAME");
					...
					sn(sb, "    public List<%s> selectBy%s(final %s) {",
							tableUEn, index1, index2);
					sn(sb, "        return selectBy%s(%s, TABLENAME);", index1,
							index3);
					sn(sb, "    }");
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql=\"SELECT %s FROM \" + TABLENAME2 + \" WHERE %s ORDER BY %s \";",
							columns3, index4, primaryKey);
					sn(sb, "            Map params = newMap();");
					for (Map<String, Object> m : idx) {
						// String INDEX_NAME = MapEx.get(m, "INDEX_NAME");
						String COLUMN_NAME = MapEx.get(m, "COLUMN_NAME");
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql=\"SELECT %s FROM ( SELECT ROWNUM r, a.* FROM \" + TABLENAME2 + \" a WHERE ROWNUM < \" + ( begin + num )+ \" AND %s ORDER BY %s ) b WHERE b.r > \" + begin + \"\";",
							columns3, index4, primaryKey);
					sn(sb, "            Map params = newMap();");
					for (Map<String, Object> m : idx) {
						// String INDEX_NAME = MapEx.get(m, "INDEX_NAME");
					...
					sn(sb, "            List<%s> result = newList();",
							primaryKeyType);
					sn(sb,
							"            String sql=\"SELECT %s FROM ( SELECT ROWNUM r, a.* FROM \" + TABLENAME2 + \" a WHERE ROWNUM < \" + ( begin + num ) + \"%s ORDER BY %s ) WHERE b.r >  \" + begin + \"\";",
							primaryKey, index4, primaryKey);
					sn(sb, "            Map params = newMap();");
					for (Map<String, Object> m : idx) {
						// String INDEX_NAME = MapEx.get(m, "INDEX_NAME");
		...
		sn(sb, "        try{");
		sn(sb,
				"            String sql = \"SELECT COUNT(*) FROM \" + TABLENAME2 + \"\";");
		sn(sb, "            return super.queryForInt(sql);");
		sn(sb, "        } catch(Exception e) {");
		sn(sb, "            log.info(e2s(e));");
		sn(sb, "            return 0;");
		...
		// SELECT * FROM ( SELECT ROWNUM r, a.* FROM 表名 a WHERE ROMNUM < m + n
		// AND 条件 ORDER BY id ) b WHERE b.r > m
		sn(sb,
				"    public List<%s> selectByPage(final int begin, final int num, final String TABLENAME2) {",
				tableUEn);
		sn(sb, "        try{");
		sn(sb,
				"            String sql = \"SELECT %s FROM ( SELECT ROWNUM r, a.* FROM \" + TABLENAME2 + \" a ORDER BY %s ) b WHERE b.r >= \" + begin + \" AND b.r < \" + ( begin + num )+ \"\";",
				columns3, primaryKey);
		sn(sb, "            return super.queryForList(sql, %s.class);",
				tableUEn);
		sn(sb, "        } catch(Exception e) {");
		...
		sn(sb, "                return -1;");
		sn(sb,
				"            String sql = \"UPDATE \" + TABLENAME2 + \" SET \" + _ustr + \" WHERE %s=:%s\";",
				primaryKey, primaryKey);
			...
			if (!column.equals(primaryKey)) {
				if (bType.contains("short") || bType.contains("int")
					...
					sn(sb,
							...
							columnUEn, bType, columnEn, pkBasicType, primaryKey);
					sn(sb, "        return update%sByKey(%s, %s, TABLENAME);",
							columnUEn, columnEn, primaryKey);
					sn(sb, "    }");
					sn(sb, "");
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s=%s+:%s WHERE %s=:%s\";",
							column, column, column, primaryKey, primaryKey);
					sn(sb, "            Map params = newMap();");
					sn(sb, "            params.put(\"%s\", %s);", primaryKey,
							primaryKey);
					...
					sn(sb,
							...
							columnUEn, pkBasicType, primaryKey, bType,
							columnEn, bType);
					sn(sb,
							"        return update%sWithMinByKey(%s, %s, _min, TABLENAME);",
							columnUEn, primaryKey, columnEn);
					sn(sb, "    }");
					sn(sb, "");
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s = (select case when %s+:%s<=:_min then :_min else %s+:%s end) WHERE %s=:%s\";",
							column, column, column, column, column, primaryKey,
							primaryKey);
					sn(sb, "            Map params = newMap();");
					sn(sb, "            params.put(\"%s\", %s);", primaryKey,
							primaryKey);
					...
					if (batch) {
						sn(sb,
								...
								columnUEn, primaryKeyType, bType, columnEn,
								bType);
						sn(sb,
						...
						sn(sb, "            String str = sb.toString();");
						sn(sb,
								"            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s = (select case when %s+:%s<=:_min then :_min else %s+:%s end) WHERE %s in (\" + str + \")\";",
					...
					sn(sb, "        try{");
					sn(sb,
							"            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s = (select case when %s+:%s>=:_max then :_max else %s+:%s end) WHERE %s=:%s\";",
							column, column, column, column, column, primaryKey,
							primaryKey);
					sn(sb, "            Map params = newMap();");
					sn(sb, "            params.put(\"%s\", %s);", primaryKey,
					...
					if (batch) {
						...
						sn(sb, "            String str = sb.toString();");
						sn(sb,
								"            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s = (select case when %s+:%s>=:_max then :_max else %s+:%s end) WHERE %s in (\" + str + \")\";",
								column, column, column, column, column,
								primaryKey);
						sn(sb, "            Map params = newMap();");
						sn(sb, "            params.put(\"_max\", _max);");
		...
		if (batch) { // 批处理
			...
			sn(sb,
					...
					tableEn, tableEn);
			sn(sb,
					"            String sql = \"UPDATE \" + TABLENAME2 + \" SET %s WHERE %s=:%s\";",
					columns8, primaryKey, primaryKey);
