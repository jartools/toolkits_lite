...
public class TkitValidateCheck {
	...
	// 验证带区号的电话
	static final Pattern p4Phone1 = Pattern.compile("^[0][1-9]{2,3}-[0-9]{5,10}$");
	// 验证没有区号的电话
	static final Pattern p4Phone2 = Pattern.compile("^[1-9]{1}[0-9]{5,8}$");
	// 验证邮件
	static final Pattern p4Email = Pattern.compile("^([a-zA-Z0-9]*[-_]?[a-zA-Z0-9]+)*@([a-zA-Z0-9]*[-_]?[a-zA-Z0-9]+)+[\\.][A-Za-z]{2,3}([\\.][A-Za-z]{2})?$");

	// 判断小数点后一位的数字的正则表达式
	static final Pattern p4Number = Pattern.compile("^(([1-9]{1}\\d*)|([0]{1}))(\\.(\\d){0,2})?$");
