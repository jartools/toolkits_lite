...
public class ExportXls {
	...
	static <T> void setThirdRow(HSSFSheet sheet, Collection<T> dataset) {
		...
		if (iterator.hasNext()) {
			...
			for (short i = 0; i < fields.length; i++) {
				...
				String typeStr = TypeEx.getBasicType(type.getName());
				HSSFRichTextString text = new HSSFRichTextString(typeStr);
				cell.setCellValue(text);
			}
		}
	}
	...
	static public <T> void exportExcel4HeadFieldType(String title,
		...
		setFirstRow(sheet, headers);

		if (dataset != null) {
			setSecondRow(sheet, dataset);
			...
			while (it.hasNext()) {
				...
				// 利用反射，根据javabean属性的先后顺序，动态调用getXxx()方法得到属性值
				Field[] fields = BeanUtils.getFields(t);
				for (short i = 0; i < fields.length; i++) {
					Field field = fields[i];
					boolean isStatic = Modifier.isStatic(field.getModifiers());
					if (isStatic) {
